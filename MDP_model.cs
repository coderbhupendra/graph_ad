...
namespace Ad_graph
{
   ...
   {  
      ...
      private double  pclick, relevance, bid; //feature list 
      ...
      public void set_transition_prob()
      {
        ...
        for (int i=0;i<no_action;i++)
        {
          ...
          {
            transition_prob[i,j]= (double)rnd.Next(1, no_nodes_linked+1)/no_nodes_linked;
          }
   ...
   class MDP 
   {
      ...
      public void learned_values()
      {
        for (int i=0;i<no_states;i++)
        {
           Console.Write("{0} {1} ",Global_nodes[i].get_state_value(),Global_nodes[i].get_best_action()); 
        }
